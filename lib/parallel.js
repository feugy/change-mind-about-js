...
export class Parallel extends Task {
   ...
   */
  toString() {
    const subTasks = this.tasks.map(t => `(${Parallel.display(t)})`).join(' | ')
   ...
   */
  async _execute(params, done) {
    if (!this.tasks.length) {
      return {[this.field]: []}
    }

    const results = await Promise.all(
      this.tasks.map(task => task.run(params))
    )
    return {
      [this.field]: results.reduce((total, current) => total.concat(current && current[this.field]), [])
    }
  }
