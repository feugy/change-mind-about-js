...
import job, {Sorter, Crawler} from '../../lib/jobs/chuck'
...
describe('Chuck Norris\'s Job', () => {
  ...
  describe('Sorter', () => {
    ...
    it('should sort incoming quotations', async () => {
      ...
      expect(await new Sorter().run({data})).to.have.property('data').that.deep.equals([data[1]])
    })
    ...
    it('should sort empty data', async () => {
      expect(await new Sorter().run({data: []})).to.have.property('data').that.is.empty
    })
  })
  ...
  describe('Crawler', () => {
    ...
    it.skip('should handle API errors', async () => {
      expect(async () => new Crawler({size: 'toto', type: 'bla'}).run()).to.throw(/Invalid argument/)
    })

    it('should return a limited number of quotes', async () => {
      const page = 1
      const size = 2
      expect(await new Crawler({page, size}).run()).to.have.property('data').that.has.lengthOf(size)
    })
  })
})
